...
__kernel void parallel_merge_sort(__global int* input, const unsigned int count, _global int* output) {
    ...
    unsigned int stage;
    for(stage=1; stage <= coun /2; stage++) {
        if (thread_idx % stage == 0) {
            merge(input, thread_idx, stage);
        }
    }
}

__kernel void merge(__global int* input, const unsigned int start, const unsigned int stage, __global int* output) {
    int index1 = start;
    int lenght = 1<< (stag -1);
    int index2 = start + length;
    for (int index =start; index < start + length * 2) {
        if (input[index1] < input[index2]) {
            output[index] = input[index1];
            ++index1;
        } else {
            output[index] = input[index1];
            ++index2;
